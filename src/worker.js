...
function normalizeReportDeliverySettings(body = {}) {
  const paymentMode = body.paymentMode === "postpaid_partial_preview" ? "postpaid_partial_preview" : "prepaid";
  const paidAmount = Math.max(0, Number(body.paidAmount || 0));
  const expectedAmount = Math.max(0, Number(body.expectedAmount || 0));
  const deliveryDueDays = Number(body.deliveryDueDays || 7);
  const safeDeliveryDueDays = Number.isFinite(deliveryDueDays)
    ? Math.min(60, Math.max(1, Math.round(deliveryDueDays)))
    : 7;
  const deliveryDueDate = String(body.deliveryDueDate || "").trim();

  return {
    paymentMode,
    paymentModeLabel: paymentMode === "postpaid_partial_preview" ? "後払い（一部だけ見せる）" : "前払い",
    paidAmount,
    expectedAmount,
    paymentStatus: expectedAmount > 0 && paidAmount >= expectedAmount ? "paid" : paidAmount > 0 ? "partial" : "unpaid",
    partialPreviewEnabled: paymentMode === "postpaid_partial_preview",
    deliveryDueDays: safeDeliveryDueDays,
    deliveryDueDate,
    sourceOfTruth: "numeria-report-delivery-snapshot",
    externalPaymentSourceOfTruth: "growth-engine-or-stripe",
  };
}
...
function createReportPdfBase64({ exportId, reportType, branding, body = {} }) {
  ...
  const deliverySettings = body.deliverySettings || normalizeReportDeliverySettings(body);
  const deliveryLine = [
    `Payment: ${deliverySettings.paymentMode}`,
    `Status: ${deliverySettings.paymentStatus}`,
    deliverySettings.deliveryDueDate ? `Due: ${deliverySettings.deliveryDueDate}` : `Due in ${deliverySettings.deliveryDueDays} days`,
    deliverySettings.partialPreviewEnabled ? "Partial preview before payment" : "Full delivery after payment confirmation",
  ].join(" / ");
  ...
  const contentLines = [
    ...
    ...pdfSection({ y: 286, label: isDetailed ? "Detailed Notes" : "Recommended Next Step", text: isDetailed ? body.notes : nextStep, maxLines: isDetailed ? 3 : 2 }),
    ...pdfSection({ y: 172, label: "Delivery Settings", text: deliveryLine, maxLines: 2 }),
...
async function handleApi(request, env = {}, ctx = null) {
  ...
  if (url.pathname === "/api/reports/export" && request.method === "POST") {
    ...
    const reportEvent = {
      ...
      branding,
      format,
      deliverySettings: reportSnapshot.deliverySettings,
      generatedAt,
    };
    ...
    enqueueApcActivity(ctx, env, {
      ...
      metadata: {
        ...
        format,
        branding,
        paymentMode: reportSnapshot.deliverySettings.paymentMode,
        paymentStatus: reportSnapshot.deliverySettings.paymentStatus,
        partialPreviewEnabled: reportSnapshot.deliverySettings.partialPreviewEnabled,
        deliveryDueDate: reportSnapshot.deliverySettings.deliveryDueDate,
        deliveryDueDays: reportSnapshot.deliverySettings.deliveryDueDays,
      },
    });
    return json({
      status: "success",

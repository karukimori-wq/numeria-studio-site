...
const bridge = `window.NumeriaD1WorkspaceLoad=window.NumeriaD1WorkspaceLoad||async function(userId){try{let response=await window.NumeriaAuthenticatedFetch(\`/api/workspace-state?workspaceId=ws_personal\`,{headers:{\"X-Workspace-Id\":\"ws_personal\"}}),body=await response.json().catch(()=>({}));if(!response.ok)return{data:null,error:{message:body.message||\"D1クラウド保存を読み込めませんでした\"}};if(body.workspaceState)return{data:body.workspaceState,error:null,source:\"d1\"};if(body.migrationRequired&&typeof X!==\"undefined\"&&X&&typeof X.from===\"function\"){let legacy=await X.from(\`numeria_workspaces\`).select(\`appraisal_profiles,saved_presets,feedback_items,app_settings\`).eq(\`user_id\`,userId).maybeSingle();if(!legacy.error&&legacy.data){let migrated=await window.NumeriaD1WorkspaceSave(legacy.data);if(!migrated.error)return{data:legacy.data,error:null,source:\"legacy-supabase-migrated-to-d1\"}}}return{data:null,error:null,source:\"d1-empty\"}}catch(error){return{data:null,error:{message:error&&error.message||\"D1クラウド保存を読み込めませんでした\"}}}};window.NumeriaD1WorkspaceSave=window.NumeriaD1WorkspaceSave||async function(workspaceState){try{let response=await window.NumeriaAuthenticatedFetch(\`/api/workspace-state\`,{method:\"PUT\",headers:{\"Content-Type\":\"application/json\",\"X-Workspace-Id\":\"ws_personal\"},body:JSON.stringify({workspaceId:\"ws_personal\",workspaceState})}),body=await response.json().catch(()=>({}));return response.ok?{data:body.workspaceState||workspaceState,error:null}:{data:null,error:{message:body.message||\"D1クラウド保存に失敗しました\"}}}catch(error){return{data:null,error:{message:error&&error.message||\"D1クラウド保存に失敗しました\"}}}};`;

writeFileSync(assetPath, bridge + patched);
console.log("Legacy Numeria workspace cloud state patched from Supabase primary storage to D1, with one-time Supabase migration fallback.");

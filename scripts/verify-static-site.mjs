...
const restoreOriginalSource = readFileSync("scripts/restore-original-site.mjs", "utf8");
...
assert.match(restoreOriginalSource, /installRomanInputFix/);
assert.match(restoreOriginalSource, /下書き保存（途中保存）/);
assert.match(restoreOriginalSource, /reading-save-helper/);
...
assert.match(workerSource, /REPORT_DELIVERY_CONTRACT_VERSION/);
assert.match(workerSource, /numeria-report-delivery-status\.v1/);
assert.match(workerSource, /\/api\/reports\/delivery-status/);
assert.match(workerSource, /reportDeliveryReadinessResponse/);
assert.match(workerSource, /createReportDeliveryStatus/);
assert.match(workerSource, /fallback_to_report_snapshot/);
...
assert.match(restoreOriginalSource, /納品状態/);
...
assert.match(cloudflareProductionWorkflow, /AUTH_ENFORCEMENT_MODE/);
assert.match(cloudflareProductionWorkflow, /verify:auth-release/);

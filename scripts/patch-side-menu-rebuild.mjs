...
const marker = "NumeriaMobileSideMenu.v2";
...
@media (width <= 760px){
  ...
  #numeria-rebuilt-side-menu{position:fixed;left:0;top:0;bottom:0;width:min(84vw,370px);z-index:239;background:#fffdf8;border-right:1px solid #e8e1d3;box-shadow:18px 0 50px rgba(25,21,39,.16);transform:translateX(-105%);transition:transform .2s ease;overflow:auto;padding:calc(40px + env(safe-area-inset-top)) 20px calc(28px + env(safe-area-inset-bottom));font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#171326}
  #numeria-rebuilt-side-menu.open{transform:translateX(0)}
  #numeria-rebuilt-side-menu header{padding:0 0 24px;border-bottom:1px solid #e7dfd0;margin-bottom:18px}
  ...
  #numeria-rebuilt-side-menu .numeria-menu-close{position:absolute;right:16px;top:calc(16px + env(safe-area-inset-top));width:38px;height:38px;border:1px solid #e3dccf;border-radius:999px;background:#fff;color:#5f5867;font-size:21px;line-height:1}
  #numeria-rebuilt-side-menu nav{display:block}
  #numeria-rebuilt-side-menu .numeria-menu-section{padding:0 0 14px;margin:0 0 14px;border-bottom:1px solid #e7dfd0}
  #numeria-rebuilt-side-menu .numeria-menu-section:last-child{border-bottom:0;margin-bottom:0}
  #numeria-rebuilt-side-menu .numeria-menu-item{display:flex;align-items:center;width:100%;min-height:50px;border:0;background:transparent;border-radius:12px;padding:8px 12px;text-align:left;color:#201b2e;font:700 16px/1.35 system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;touch-action:manipulation}
...
const MARK="NumeriaMobileSideMenu.v2";
...
function openMenu(){refreshState();var menu=document.getElementById(MENU_ID);var backdrop=document.getElementById(BACKDROP_ID);if(menu)menu.classList.add("open");if(backdrop)backdrop.classList.add("open");document.documentElement.classList.add("numeria-side-menu-open")}
...
function navigatePage(item){var api=nav();if(!api||typeof api.go!=="function"){setStatus("画面遷移の準備中です。もう一度お試しください。");return}closeMenu();api.go(item.page);if(item.focus)setTimeout(function(){waitForElement(item.focus,0)},30)}
...
function installToggle(){var appbar=document.getElementById("numeria-mobile-appbar");if(!appbar)return false;var current=appbar.querySelector(".mobile-menu-toggle");if(!current)return false;if(current===installedToggle)return true;var replacement=current.cloneNode(true);current.replaceWith(replacement);replacement.addEventListener("click",function(event){event.preventDefault();event.stopPropagation();openMenu()});installedToggle=replacement;return true}
function refreshState(){var api=nav();var role=api&&typeof api.getRole==="function"?api.getRole():"";var page=api&&typeof api.getPage==="function"?api.getPage():"";var menu=document.getElementById(MENU_ID);if(!menu)return;Array.from(menu.querySelectorAll("[data-admin-only]")).forEach(function(button){button.hidden=role!=="admin"});Array.from(menu.querySelectorAll(".numeria-menu-item")).forEach(function(button){var id=button.dataset.menuId;var item=null;MENU_SECTIONS.some(function(section){item=section.find(function(candidate){return candidate.id===id})||null;return !!item});button.classList.toggle("active",!!(item&&item.page&&item.page===page))})}
function install(){if(!window.matchMedia("(max-width: 760px)").matches)return;removeLegacyMenu();renderMenu();installToggle();refreshState()}
function start(){install();var observer=new MutationObserver(function(){install()});observer.observe(document.body,{childList:true,subtree:true});setInterval(function(){install();refreshState()},1200)}
...
console.log("Mobile side menu rebuilt with direct Navigation API and a single menu configuration.");

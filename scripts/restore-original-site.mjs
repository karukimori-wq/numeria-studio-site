...
@media (width <= 760px){
  ...
  .mobile-notice-backdrop{background:rgba(22,18,37,.42);display:none;inset:0;position:fixed;z-index:210}
  .mobile-notice-backdrop.open{display:block}
  .mobile-notice-panel{background:#fffdf8;border:1px solid #e3dacb;border-radius:20px 20px 0 0;bottom:0;box-shadow:0 -18px 60px rgba(25,19,42,.2);display:grid;gap:13px;left:0;max-height:82vh;overflow:auto;padding:18px 18px calc(34px + env(safe-area-inset-bottom));position:fixed;right:0;transform:translateY(105%);transition:transform .2s ease;z-index:220}
  .mobile-notice-panel.open{transform:translateY(0)}
  .mobile-notice-panel header{align-items:flex-start;display:flex;gap:12px;justify-content:space-between}
  .mobile-notice-panel h2{font-family:Georgia,Yu Mincho,serif;font-size:24px;font-weight:500;line-height:1.25;margin:0}
  .mobile-notice-panel p{color:#746b62;font-size:13px;line-height:1.7;margin:0}
  .mobile-notice-close{background:#fff;border:1px solid #ded7ca;border-radius:50%;color:#2a2338;font-size:18px;height:36px;width:36px}
  .mobile-notice-list{display:grid;gap:8px}
  .mobile-notice-item{background:#fff;border:1px solid #e4dac9;border-radius:13px;padding:12px}
  .mobile-notice-item span{color:#9d7737;display:block;font-size:11px;font-weight:900;letter-spacing:.08em;margin-bottom:5px}
  .mobile-notice-item strong{color:#2a2338;display:block;font-size:15px;margin-bottom:4px}
  .mobile-notice-item small{color:#746b62;display:block;font-size:12px;line-height:1.6}
...
const mobileNoticePatch = `<script>window.NumeriaInstallMobileNoticeSheet=function(){if(window.__numeriaMobileNoticeSheetInstalled)return;window.__numeriaMobileNoticeSheetInstalled=true;function ensureNoticePanel(){var existing=document.querySelector(".mobile-notice-panel");if(existing)return existing;var backdrop=document.createElement("div");backdrop.className="mobile-notice-backdrop no-print";document.body.appendChild(backdrop);var panel=document.createElement("section");panel.className="mobile-notice-panel no-print";panel.setAttribute("aria-label","お知らせ");panel.innerHTML='<header><div><h2>お知らせ</h2><p>Numeria Studioからの更新、メンテナンス、Pro機能追加をまとめます。</p></div><button class="mobile-notice-close" type="button" aria-label="閉じる">×</button></header><div class="mobile-notice-list"><article class="mobile-notice-item"><span>NEW FEATURE</span><strong>下部メニューを5項目に整理</strong><small>ホーム、カルテ、鑑定、鑑定書、サポートへ整理しました。</small></article><article class="mobile-notice-item"><span>SUPPORT</span><strong>問い合わせ入口を改善</strong><small>不具合報告、改善要望、問い合わせを右上と下部メニューから送れます。</small></article><article class="mobile-notice-item"><span>PRO</span><strong>鑑定書まわりを拡張中</strong><small>詳細レポート、ブランド表示、納品条件の整理を進めています。</small></article><article class="mobile-notice-item"><span>STATUS</span><strong>障害情報</strong><small>現在、アプリ本体の公開先では主要チェックが正常です。</small></article></div>';document.body.appendChild(panel);function close(){panel.classList.remove("open");backdrop.classList.remove("open")}backdrop.addEventListener("click",close);panel.querySelector(".mobile-notice-close").addEventListener("click",close);return panel}function openNoticePanel(){var panel=ensureNoticePanel();var backdrop=document.querySelector(".mobile-notice-backdrop");panel.classList.add("open");if(backdrop)backdrop.classList.add("open")}document.addEventListener("click",function(event){var button=event.target.closest&&event.target.closest(".mobile-notice");if(!button)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();openNoticePanel()},true)};window.NumeriaInstallMobileNoticeSheet();</script>`;

const authTokenBridgePatch = `<script>window.NumeriaInstallAuthTokenBridge=function(){if(window.__numeriaAuthTokenBridgeInstalled)return;window.__numeriaAuthTokenBridgeInstalled=true;var installer=window.NumeriaInstallAuthBridge;if(typeof installer!=="function")return;window.NumeriaInstallAuthBridge=function(client){installer(client);if(!client||!client.auth||client.auth.__numeriaTokenBridge)return;client.auth.__numeriaTokenBridge=true;async function attachToken(result){try{var session=result&&result.data&&result.data.session;if(!session||!window.Clerk||!window.Clerk.session||typeof window.Clerk.session.getToken!=="function")return result;var token=await window.Clerk.session.getToken();if(token){session.access_token=token;session.refresh_token=token;session.provider_token=token}}catch(error){}return result}function wrapResult(name){var original=client.auth[name];if(typeof original!=="function")return;client.auth[name]=async function(){return attachToken(await original.apply(client.auth,arguments))}}wrapResult("getSession");wrapResult("signInWithPassword");wrapResult("signUp");var originalListener=client.auth.onAuthStateChange;if(typeof originalListener==="function"){client.auth.onAuthStateChange=function(callback){return originalListener.call(client.auth,async function(event,session){var wrapped=await attachToken({data:{session:session},error:null});callback(event,wrapped.data.session)})}}}};window.NumeriaInstallAuthTokenBridge();</script>`;

const productionHtml = originalHtml
  .replace(unsupportedStrategyGuard, "")
  .replace("</head>", `${adminReadinessPanel}${mobileInformationArchitecturePatch}${mobileNoticePatch}${authTokenBridgePatch}</head>`);
